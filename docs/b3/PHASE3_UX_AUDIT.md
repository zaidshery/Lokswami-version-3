...
### 2.2 Server/Client Boundary Observations
...
### 3.1 Canonical Reader Routes (`/main/...`)
...
### 4.2 Content Cards & Presentation Components
...
### 4.3 Breaking News & Engagement Components
...
## 5. Visual Brand & Style Debt

### 5.1 The Orange/Red Palette Clash
...
### 5.2 Competing Dark Mode Backgrounds
...
### 6.1 Top Stacking Offset Fragility (P1)
...
### 6.2 Mobile Horizontal Gesture Collision (P1)
...
### 6.3 Desktop Navigation Overflow (P2)
...
### 6.4 Bottom Nav Sizing & Matra Illegibility on Mobile (P2)
...
## 7. Surface-by-Surface Audit Findings
...
### 7.2 Article Reader (`/main/article/[id]`)
...
### 7.3 Video Hub (`/main/videos`) & Shorts (`/main/ftaftaf`)
...
### 7.4 E-Paper Viewer (`/main/epaper`)
...
### 7.6 Reader Account & Preferences (`/main/account`, `/main/preferences`)
...
### 7.7 Universal Share System (`components/ui/ShareMenu.tsx`)
...
## 8. Hindi Typography First-Class Audit
...
## 9. Mobile-First & Responsive Evaluation

### 9.1 Evaluation Across Standard Viewports
| Width | Device Class | Key Findings & Usability Issues |
|---|---|---|
...
## 10. Accessibility (a11y) Findings
...
## 11. Performance & Core Web Vitals (CWV) Risks
...
## 12. Prioritized Findings (P0 / P1 / P2 / P3)
...
| **P1** | UX-003 | Design System | Tailwind `orange` palette is aliased to red (`#e72129`), causing massive semantic confusion, mixed utility classes, and inability to use legitimate warm secondary accents. | Decouple color tokens in Phase 3.2. Centralize authentic LokSwami brand tokens (`brand`, `brand-hover`, `breaking`, `charcoal`, `neutral`). |
| **P2** | UX-004 | Dark Mode | Four competing dark mode background definitions (`#242024`, `#020617`, `#050608`, `#18181b`) cause visual seams and banding. | Standardize dark mode tokens in Phase 3.2 around a unified, cohesive editorial dark surface scale. |
...
## 13. Recommended Phase 3 Implementation Sequence
...
1. **Phase 3.2 — LokSwami Design System Foundation (CURRENT WORK)**:
   ...
   - Resolve the orange aliasing debt with backwards-compatible clean tokens.
   - Establish Hindi typography scale (Devanagari first, line-height 1.6+ for body, no negative tracking).
   - Define editorial reading containers (`max-w-reading` 68ch, article container 840px).
   - Build accessible core primitives: `Button`, `Badge`, `BreakingBadge`, `SectionHeader`, `Container`, `ReaderPageShell`, `EmptyState`, `ErrorState`, `Skeleton`, `MetadataRow`.
