...
- **Findings**:
  ...
  - **Granular Audience Persistence Semantics**:
    - **Newsletter Subscriptions**: `Subscriber` is **MongoDB-only**. There is NO JSON fallback. If `MONGODB_URI` is unconfigured or unreachable, `audienceCaptureService.subscribe()` returns HTTP 503 (`Subscription service is not configured yet`).
...
| **Audience: Subscriptions** | MongoDB (`Subscriber`) | **None** (NO JSON fallback) | **None** (MongoDB only) | No (Single store) | No | No | **Fail-closed** (HTTP 503 if unconfigured/offline) |
