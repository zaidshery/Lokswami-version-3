...
- **Findings**:
  ...
  - **Granular Audience Persistence Semantics**:
    - **Newsletter Subscriptions**: `Subscriber` is **MongoDB-only** with zero file fallback. If `MONGODB_URI` is missing, `audienceCaptureService.subscribe()` returns HTTP 503 (`{ success: false, error: 'Subscription service is not configured yet' }`). If MongoDB is configured but connection or database operations throw, the exception bubbles uncaught to `app/api/subscribe/route.ts` whose catch block returns HTTP 500 (`{ success: false, error: 'Failed to subscribe. Please try again.' }`). In neither case is any file-based subscriber record created.
    - **Marketing Leads**: `audienceRepository.saveMarketingLead` is **Mongo-first then JSON fallback**. Attempts MongoDB `MarketingLead.findOneAndUpdate()`; if Mongo throws, the error is caught and input is persisted to `data/marketing-leads.json`.
    - **Commercial Inquiries**: `audienceRepository.createAdvertiseInquiry` is **Mongo-first then JSON fallback**. Attempts MongoDB `AdvertiseInquiry.create()`; if Mongo throws, catches error and persists to `data/advertise-inquiries.json`.
    - **Career Applications**: `audienceRepository.createCareerApplication` is **Mongo-first then JSON fallback**. Attempts MongoDB `CareerApplication.create()`; if Mongo throws, catches error and persists to `data/career-applications.json`.
    - **Contact Messages & Workflow**: `ContactRepository` is **Mongo-first then JSON fallback**. Attempts MongoDB `ContactMessage` first; on error, catches and falls back to `data/contact-messages.json` for submission writes (`create`), workflow updates (`update`), detail lookups (`getById`), and inbox queries (`list`).
    - **Elections**: Dedicated **local file and graphic asset storage**. Results are stored and normalized in `data/election-results.json` via `lib/elections/storage.ts`. State visual graphics are managed directly on the filesystem in `public/elections/${stateId}.jpg` via `ElectionAssetRepository`. Zero MongoDB collection.
  ...
  - **Social Post Dispatch State Machine & Human Gate**:
...
| **Content (Newsroom Writes)** | MongoDB (`Article`, `ArticleRevision`, `ArticleLock`) | File (`data/articles.json`) | Pinned on start (`resolveNewsroomArticleStore`) | **No** (Strict pinning) | No | No | Graceful degradation with store pinning |
| **Video & Swipe** | MongoDB (`Video`) | File (`data/videos.json`) | Pinned on start (`resolveStore`) | **No** (Strict pinning) | No | No | Graceful degradation with store pinning |
...
| **Audience: Subscriptions** | MongoDB (`Subscriber`) | **None** (NO JSON fallback) | **None** (MongoDB only) | No (Single store) | No | No | **Fail-closed** (HTTP 503 if MONGODB_URI missing; HTTP 500 on DB exception in route catch block) |
| **Audience: Marketing Leads** | MongoDB (`MarketingLead`) | None | File (`data/marketing-leads.json`) via catch block | N/A (Inline catch fallback) | No | No | Graceful degradation (JSON fallback on Mongo error) |
| **Audience: Commercial Inquiries** | MongoDB (`AdvertiseInquiry`) | None | File (`data/advertise-inquiries.json`) via catch block | N/A (Inline catch fallback) | No | No | Graceful degradation (JSON fallback on Mongo error) |
| **Audience: Career Applications** | MongoDB (`CareerApplication`) | None | File (`data/career-applications.json`) via catch block | N/A (Inline catch fallback) | No | No | Graceful degradation (JSON fallback on Mongo error) |
| **Audience: Contact Messages & Workflow** | MongoDB (`ContactMessage`) | File (`data/contact-messages.json`) on error | File (`data/contact-messages.json`) via catch block | N/A (Inline catch fallback) | No | No | Graceful degradation (JSON fallback on Mongo error for create, update, detail, list) |
| **Audience: Elections** | Local filesystem (`data/election-results.json`, `public/elections/*.jpg`) | Native file | Native file | No (File authority) | No | No | Direct local file & graphic asset authority (no MongoDB) |
| **Social Distribution** | MongoDB (`SocialPost`) | File (`data/social-posts.json`) | Pinned on start (`resolveStore`) | **No** (Strict pinning) | No | Webhook (n8n/automation) | Graceful degradation with store pinning |
| **Analytics Telemetry** | MongoDB (`AnalyticsEvent`) | File (`data/analytics-events.json`) | Appends to disk on error | N/A (Inline catch fallback) | No | No | Graceful degradation (non-blocking) |
| **Media Metadata** | MongoDB (`Media`) | File (`data/media.json`) | File if Mongo unconfigured | No | No | DO Spaces (blobs) | Graceful degradation for metadata |
...
- **Social Posts**: Require explicit `admin` or `super_admin` action on `/api/admin/social-posts/[id]/dispatch`. The post must be in `approved`, `scheduled`, or `failed` (retry) status. Drafts/unapproved items are strictly rejected (HTTP 400). Re-dispatching a failed post simply retries automation for an already approved post without bypassing human approval. Zero autonomous publication.
...
- **Write-Store Pinning**: Evaluates store target once at mutation start and stays pinned throughout multi-step operations for Content Newsroom Writes (`resolveNewsroomArticleStore`), Video & Swipe (`VideoRepository.resolveStore`), and Social Distribution (`SocialPostRepository.resolveStore`). Public capture pipelines (Audience capture, Analytics telemetry) catch Mongo errors to degrade gracefully to secondary files without multi-step store pinning.
