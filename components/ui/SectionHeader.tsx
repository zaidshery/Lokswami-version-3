  ...
  return (
    <div className={`mb-3 flex min-w-0 items-center justify-between gap-3 ${className}`}>
      ...
      {href && ctaText ? (
        <Link
          href={href}
          className="editorial-focus-ring inline-flex min-h-[44px] min-w-[44px] shrink-0 items-center gap-1 rounded-editorial-sm px-2 text-xs font-bold text-brand-500 transition motion-reduce:transition-none hover:bg-brand-50 hover:text-brand-600 dark:text-brand-400 dark:hover:bg-brand-950/40 dark:hover:text-brand-300"
        >
          <span>{ctaText}</span>
